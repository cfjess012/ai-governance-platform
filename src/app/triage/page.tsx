...
export default function TriagePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return triageQueue.filter((uc) => {
      ...
      if (riskFilter === 'high') return tier === 'high' || tier === 'critical';
  ...
  const stats = useMemo(() => {
    ...
    const high = triageQueue.filter((uc) => {
      const t = uc.classification.riskTier;
                    ...
                    <tr key={uc.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">
                          {uc.intake.businessArea} · {uc.intake.aiType?.replace(/_/g, ' ')}
                        </p>
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${riskTierColors[uc.classification.riskTier] ?? riskTierColors.pending}`}
                        >
                          {uc.classification.riskTier === 'pending'
                            ? 'Pending'
                            : uc.classification.riskTier.charAt(0).toUpperCase() +
                              uc.classification.riskTier.slice(1)}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span
